...
调研日期：2026-09-11。以下第 1–4 节保留 v0.1 企业 Agent 课程的历史调研与当时的章节映射，不能直接套用 v0.2 章号。源码快照未重新核验，运行验证仍未完成。当前课程以[蓝图 v0.2](../course-blueprint.md)为准。

目标更新：为通用 Harness 与开发者 Agent 主线选择少量源码参考，按当前问题阅读，避免将多个框架同时设为必装依赖。
...
课程主线已改为通用 Harness 与开发者 Agent，原“三个核心框架”的定位相应调整：

| 参考 | 新定位 | 待完成验证 |
|---|---|---|
| Pi | 运行循环、消息、工具执行的主要源码参考 | 固定版本下最小模型/工具调用及错误传播 |
| mini-swe-agent | 开发者 Agent 简单基线的补充参考 | 小型仓库任务与预算/终止行为；Python 源码按需解释 |
| Mastra | 工作流和企业迁移专题对照 | 同一业务操作的暂停、恢复及外部副作用语义 |
| Deep Agents JS | 上下文、子任务和组合机制专题对照 | 相关中间件调用链、隔离与对照实验 |
| nanobot、OpenClaw、Hermes | 记忆、长期运行或产品专题的候选资料 | 按具体问题决定是否深入，不增加整仓阅读任务 |

DeepSeek Harness 与 Kimi Code 可列为后续成熟运行时对照候选，但本调研文档尚未核验其源码快照、许可与运行行为，不将其描述为已分析完成。纳入详细章节前再做官方来源和固定版本验证。

下一交付是第 01 章首个小节及阶段一分步结构。后续源码精读围绕当章问题进行，记录包版本、模型配置、操作系统、实际结果与证据边界，不用历史 Stars 决定教学顺序。
